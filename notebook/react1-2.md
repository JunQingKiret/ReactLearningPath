...
或许我们可以输出输入的文本，event.target.value可以获取输入的值，在浏览器的控制台可以查看输出的值。
...
首先我们需要通过一个字符变量inputText，与我们handleChange函数中的changedText进行绑定
...
绑定成功后，接着我们将对原有的p标签元素进行修改，而这里需要用到React中的函数useState。
...
通过定义的stage状态变量，我们可以获取到更新变化的值，在这个例子中可以获取到按钮按下的次数，并做出反应
